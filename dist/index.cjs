...
var ort__namespace = /*#__PURE__*/_interopNamespaceDefault(ort);
...
class ONNXWebFramework {
  ...
  async initialize() {
    try {
      ...
      if (this.options.wasmPaths && typeof this.options.wasmPaths === 'string') {
        ort__namespace.env.wasm.wasmPaths = this.options.wasmPaths;
  ...
  async loadModel(name, modelSource, sessionOptions = {}) {
    ...
    try {
      ...
      const session = await ort__namespace.InferenceSession.create(modelArray, finalOptions);
